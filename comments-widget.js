class CommentsWidget extends HTMLElement {
  ...
  connectedCallback() {
    ...
    window.addEventListener('message', (event) => {
      if (event.data.type === 'setComments') {
        this.comments = event.data.comments || [];
        this.renderComments();
      }
      if (event.data.type === 'setStats') {
        this.updateStats(event.data.stats);
      }
    });
  ...
  setupEventListeners() {
    ...
    $('fbShare').onclick = () => this.sendMessage('share', { platform: 'facebook' });
    $('xShare').onclick = () => this.sendMessage('share', { platform: 'x' });
    $('linkedinShare').onclick = () => this.sendMessage('share', { platform: 'linkedin' });
    $('copyLink').onclick = () => this.sendMessage('share', { platform: 'copy' });
    $('articleLikeBtn').onclick = () => this.sendMessage('likeArticle', {});
    $('submitBtn').onclick = () => this.submitComment();
    $('cancelBtn').onclick = () => this.cancelComment();
    $('cancelReplyBtn').onclick = () => this.cancelReply();
    $('sortSelect').onchange = () => this.sendMessage('sortChange', { sort: $('sortSelect').value });
  }
  ...
  submitComment() {
    ...
    this.sendMessage('submitComment', { name: name || '訪客', email, content, parentId: this.replyingTo?.id || null, replyToName: this.replyingTo?.name || null });
    this.cancelComment();
  }
  ...
  renderComments() {
    ...
    this.comments.forEach(item => {
      ...
      div.innerHTML = `
        <div class="avatar"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
        <div class="comment-content">
          <div class="comment-header"><div><div class="comment-name">${this.escapeHtml(item.name) || '匿名'}</div><div class="comment-date">${this.formatDate(item._createdDate)}</div></div></div>
          ...
          <div class="comment-actions"><button class="action-btn like-btn" data-id="${item._id}">♡ 按讚${item.likes ? ' ' + item.likes : ''}</button><button class="action-btn reply-btn" data-id="${item._id}" data-name="${this.escapeHtml(item.name) || '匿名'}">↩ 回覆</button></div>
        </div>`;
      container.appendChild(div);
    });
    container.querySelectorAll('.like-btn').forEach(btn => { btn.onclick = () => this.sendMessage('likeComment', { commentId: btn.dataset.id }); });
    container.querySelectorAll('.reply-btn').forEach(btn => { btn.onclick = () => this.startReply(btn.dataset.id, btn.dataset.name); });
    const count = this.comments.length;
    this.shadowRoot.getElementById('commentCount').textContent = `${count} 則留言`;
    this.shadowRoot.getElementById('commentCountTop').textContent = `${count} 則留言`;
  }

  updateStats(stats) {
    this.shadowRoot.getElementById('viewCount').textContent = `${(stats.views || 0).toLocaleString()} 次查看`;
    this.shadowRoot.getElementById('articleLikeNum').textContent = stats.likes || 0;
  }

  formatDate(dateStr) { if (!dateStr) return ''; const now = new Date(); const d = new Date(dateStr); const days = Math.floor((now - d) / (1000 * 60 * 60 * 24)); if (days === 0) return '今天'; if (days === 1) return '1 天前'; if (days < 7) return `${days} 天前`; return `${d.getMonth() + 1}月${d.getDate()}日`; }
  escapeHtml(text) { if (!text) return ''; return text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }
}
